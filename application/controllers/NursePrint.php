...
class NursePrint extends CI_Controller {
    ...
    public function nurse_fpdf($param1 = '') {
			if ($this->session->userdata('admin_login') != 1) {
				$this->session->set_userdata('last_page', current_url());
				redirect(base_url(), 'refresh');
			}
		$nurse_info = $this->crud_model->select_nurse_info_by_id($param1);
		foreach ( $nurse_info as $val ):		
			...
			$address = $this->db->get_where( 'settings', array( 'type' => 'address' ))->row()->description;
                ...
                $this->header_info( '', $val['nurse_id'], $system_title, 'logo', $contact, $address );

                $this->fpdf->Ln(1);
                $this->header_design( 'Name', ': ' . $val['name'], 'ID', ': ' . $val['nurse_id'] );
                $this->header_design( 'Dasignation', ': Nurse', 'Mobile', ': ' . $val['phone'] );
                $this->fpdf->Ln(5);
    ...
    public function header_info($barcode_text = '', $sid = '', $title_tab = '', $logo = '', $contact = '', $address = '') {
		...
		$string = read_file('uploads/nurse_image/'.$sid.'.jpg');
        if ($string) {
             $this->fpdf->Image('uploads/nurse_image/'.$sid.'.jpg', 179, 28, 16, 16);
        }
