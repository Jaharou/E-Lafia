...
class AccountantPrint extends CI_Controller {
    ...
    public function accountant_fpdf($param1 = '') {
			if ($this->session->userdata('admin_login') != 1) {
				$this->session->set_userdata('last_page', current_url());
				redirect(base_url(), 'refresh');
			}
		$accountant_info = $this->crud_model->select_accountant_id_info_by_id($param1);
		foreach ( $accountant_info as $val ):		
			...
			$address = $this->db->get_where( 'settings', array( 'type' => 'address' ))->row()->description;
                ...
                $this->header_info( '', $val['accountant_id'], $system_title, 'logo', $contact, $address );

                $this->fpdf->Ln(1);
                $this->header_design( 'Name', ': ' . $val['name'], 'ID', ': ' . $val['accountant_id'] );
                $this->header_design( 'Dasignation', ': Accountant', 'Mobile', ': ' . $val['phone'] );
                $this->fpdf->Ln(5);
				...
				$this->fpdf->Ln();  
		endforeach;				
				$this->fpdf->Output( 'Accountant.pdf', 'D');
    }
