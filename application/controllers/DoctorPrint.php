...
class DoctorPrint extends CI_Controller {
    ...
    public function doctor_fdf($param1 = '') {
			if ($this->session->userdata('admin_login') != 1) {
				$this->session->set_userdata('last_page', current_url());
				redirect(base_url(), 'refresh');
			}
		$doctor_info = $this->crud_model->select_doctor_info_by_id($param1);
		foreach ( $doctor_info as $val ):		

			$system_title = $this->db->get_where('settings', array('type' => 'system_title'))->row()->description;              
			$contact = $this->db->get_where('settings', array('type' => 'phone'))->row()->description;
			$address = $this->db->get_where('settings', array('type' => 'address'))->row()->description;
			$department = $this->db->get_where('department' , array('department_id' => $val['department_id'] ))->row()->name;
                ...
                $this->header_info('', $val['doctor_id'], $system_title, 'logo', $contact, $address);

                $this->fpdf->Ln(1);
                $this->header_design('Name', ': ' . $val['name'], 'Doctor ID', ': ' . $val['doctor_id']);
                $this->header_design('Department', ': ' . $department, 'Mobile', ': ' . $val['phone']);
                $this->fpdf->Ln(5);

                $this->body_design("Email", $val['email']);
                $this->body_design("Address", $val['address']);
                $this->body_design("Profile", $val['profile']);
          
				$this->fpdf->Ln();  
		endforeach;				
				$this->fpdf->Output( 'doctors.pdf', 'D');
    }
    ...
    public function header_info($barcode_text = '', $sid = '', $title_tab = '', $logo = '', $contact = '', $address = '') {
		...
		$string = read_file('uploads/doctor_image/'.$sid.'.jpg');
        if ($string) {
             $this->fpdf->Image('uploads/doctor_image/'.$sid.'.jpg', 179, 28, 16, 16);
        }
