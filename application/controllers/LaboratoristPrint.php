...
class LaboratoristPrint extends CI_Controller {
    ...
    public function laboratorist_fpdf($param1 = '') {
			if ($this->session->userdata('admin_login') != 1) {
				$this->session->set_userdata('last_page', current_url());
				redirect(base_url(), 'refresh');
			}
		$laboratorist_info = $this->crud_model->select_laboratorist_id_info_by_id($param1);
		foreach ( $laboratorist_info as $val ):		
			...
			$address = $this->db->get_where( 'settings', array( 'type' => 'address' ))->row()->description;
                ...
                $this->header_info( '', $val['laboratorist_id'], $system_title, 'logo', $contact, $address );

                $this->fpdf->Ln(1);
                $this->header_design( 'Name', ': ' . $val['name'], 'ID', ': ' . $val['laboratorist_id'] );
                $this->header_design( 'Dasignation', ': Laboratorist', 'Mobile', ': ' . $val['phone'] );
                $this->fpdf->Ln(5);
				...
				$this->fpdf->Ln();  
		endforeach;				
				$this->fpdf->Output( 'Laboratorists.pdf', 'D');
    }
    ...
    public function header_info($barcode_text = '', $sid = '', $title_tab = '', $logo = '', $contact = '', $address = '') {
		...
		$string = read_file('uploads/laboratorist_image/'.$sid.'.jpg');
        if ($string) {
             $this->fpdf->Image('uploads/laboratorist_image/'.$sid.'.jpg', 179, 28, 16, 16);
        }
