...
class BloodDonorPrint extends CI_Controller {
    ...
    public function dhorla_fpdf($param1 = '') {
			if (  $this->session->userdata('nurse_login') == '1' || $this->session->userdata('laboratorist_login') == '1') {
			
		$blood_donor_info = $this->crud_model->select_blood_donor_info_by_id($param1);
		foreach ( $blood_donor_info as $val ):		
			...
			$address = $this->db->get_where( 'settings', array( 'type' => 'address' ))->row()->description;
				...
				$this->fpdf->Ln(4);				
                $this->header_info( '', $system_title, 'logo', $contact, $address );
                $this->fpdf->Ln(1);
				
                $this->header_design( 'Name', ': ' . $val['name'], 'Doner ID', ': ' . $val['blood_donor_id'] );
                $this->header_design( 'Last Donate', ': ' .date('d/m/Y', $val['last_donation_timestamp']), 'Mobile', ': ' . $val['phone'] );
                $this->fpdf->Ln(5);
		...
		endforeach;				
				$this->fpdf->Output( 'blood_donor.pdf', 'D');					
		}
    }
    ...
    public function header_info($barcode_text = '', $title_tab = '', $logo = '', $contact = '', $address = '') {
