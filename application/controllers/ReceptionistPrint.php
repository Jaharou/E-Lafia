...
class ReceptionistPrint extends CI_Controller {
    ...
    public function receptionist_fpdf($param1 = '') {
			if ($this->session->userdata('admin_login') != 1) {
				$this->session->set_userdata('last_page', current_url());
				redirect(base_url(), 'refresh');
			}
		$receptionist_info = $this->crud_model->select_receptionist_info_by_id($param1);
		foreach ( $receptionist_info as $val ):		
			...
			$address = $this->db->get_where( 'settings', array( 'type' => 'address' ))->row()->description;
                ...
                $this->header_info( '', $val['receptionist_id'], $system_title, 'logo', $contact, $address );

                $this->fpdf->Ln(1);
                $this->header_design( 'Name', ': ' . $val['name'], 'ID', ': ' . $val['receptionist_id'] );
                $this->header_design( 'Dasignation', ': Receptionist', 'Mobile', ': ' . $val['phone'] );
                $this->fpdf->Ln(5);
				...
				$this->fpdf->Ln();  
		endforeach;				
				$this->fpdf->Output( 'Receptionists.pdf', 'D');
    }
